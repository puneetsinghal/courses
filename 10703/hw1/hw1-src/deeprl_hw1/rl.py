...
def evaluate_policy_sync(env, gamma, policy, max_iterations=int(1e3), tol=1e-3):
	...
	for itr in range (max_iterations):
		...
		if(delta<tol):
			break
		# if(np.sum(np.fabs(newValue-value_func)) < tol):
...
def policy_iteration_sync(env, gamma, max_iterations=int(1e3), tol=1e-3):
	...
	for numPolicyIterations in range(max_iterations):
		...
		if(policyStable):
			return policy, value_func, numPolicyIterations, numValueIterations
...
def value_iteration_sync(env, gamma, max_iterations=int(1e3), tol=1e-3):
	...
	for numValueIterations in range (max_iterations):
		...
		for state in range(env.nS):
			...
			for action in range(env.nA):
				# prob, nextState, reward, is_terminal = (env.P[state][action])[0]
				# if not is_terminal:
				# 	temp = max(temp, 1.0*prob*(reward + gamma*value_func[nextState]))
				# else:
				# 	temp = max(temp, 1.0*prob*(reward))
	...
	return policy, value_func, numValueIterations
	# return np.zeros(env.nS), 0
...
def value_iteration_async_ordered(env, gamma, max_iterations=int(1e3), tol=1e-3):
	...
	return policy, numValueIterations
	# return np.zeros(env.nS), 0
...
def value_iteration_async_custom(env, gamma, max_iterations=int(1e3), tol=1e-3):
	...
	# previousQueue = Queue.PriorityQueue()
	# numStateEvaluations = 0
	# numStableValues = 0

	# for state in range(env.nS):
	# 	previousQueue.put((0., 1, state))

	# for numValueIterations in range (max_iterations):
	# 	nextQueue = Queue.PriorityQueue()
	# 	# print(openQueue.queue)
	# 	delta = 0
	# 	for i in range(env.nS):
	# 		numStateEvaluations +=1
	# 		priorityValue, entry_count, state = previousQueue.get()
	# 		# for state in randomPermStates:
	# 		old_value = value_func[state]
	# 		temp = 0
	# 		for action in range(env.nA):
	...
	# 		nextQueue.put((delta, entry_count+1, state))
	# 		# print(state, delta, numStableValues)
	# 	if(delta<tol):
	# 		break
	# 	else:
	# 		previousQueue = copy(nextQueue)
