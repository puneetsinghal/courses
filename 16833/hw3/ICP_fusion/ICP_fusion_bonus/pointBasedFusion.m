function [fusion_map, next_ref_data] = pointBasedFusion(fusion_map, input_data, tform, cam_param, sigma, ds_ratio, t)
    ...
    trans_colors = double(trans_pointcloud.Color);
    ...
    %==== Find if each input point is a newly observed point ====
    is_color = isColorCloseToProjPoints(trans_colors, proj_colors, dist2_th);
    ...
    %==== Removing points based on confidence and time last updated ====
    fusion_map = removingPoints(fusion_map, C_stable, t, T_max);
