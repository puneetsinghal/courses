            ...
            int moveActionIndex, int moveToTableActionIndex, int*** plan, int* planlength) 
{
	...
	*planlength = 0;
	...
	graph masterGraph(blocksV, numofblocks, trianglesV, numoftriangles,
		TableIndex, onV_start, onV_start_length, clearV_start, numofclear_start,
        onV_goal, onV_goal_length, clearV_goal, numofclear_goal);
	// masterGraph.print_graph();
    ...
    std::list<literalsStruct*> pathVertices;

    while(currentNode->parentAddress!=NULL)
    {
    	// masterGraph.print_vertex(*currentNode);
    	pathVertices.push_back(currentNode);
    	currentNode = currentNode->parentAddress;
    }
    numofsteps = pathVertices.size();
    *plan = (int**) malloc(numofsteps*sizeof(int*));
    int i = numofsteps-1;
    for(std::list<literalsStruct*>::iterator it=pathVertices.begin(); it!=pathVertices.end(); it++)
    {
