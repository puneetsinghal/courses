...
def main():
  ...
  for step in range(max_iter):
    ...
    [output, cp, param_grad] = cnn_lenet.conv_net(params,
                                          layers,
                                          xtrain[:, idx],
                                          ytrain[idx], True)
    ...
    if (step+1) % test_interval == 0:
      ...
      _, cptest, _ = cnn_lenet.conv_net(params, layers, xtest, ytest, False)
      layers[1]['batch_size'] = 64
  ...
  plt.figure()
  plt.plot(np.linspace(1, numSamples, numSamples), costResults[0,:], color='k', linewidth=3, label='train cost')
  plt.plot(np.linspace(1, numSamples, numSamples), costResults[1,:], color='r', linewidth=3,  linestyle=':', label='test cost')
  ...
  plt.figure()
  plt.plot(np.linspace(1, numSamples, numSamples), accuracyResults[0,:], color='k', linewidth=3, label='train accuracy')
  plt.plot(np.linspace(1, numSamples, numSamples), accuracyResults[1,:], color='r', linewidth=3,  linestyle=':', label='test accuracy')
